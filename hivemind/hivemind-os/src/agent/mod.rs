...
/// Evaluate all agents and write action blobs for fired rules.
/// Lock ordering: RUNTIME → HIVE (never the reverse). Safe in single-threaded kernel.
pub fn tick_all() {
    // Phase 1 — snapshot rules without holding RUNTIME while touching HIVE.
    let snapshots: Vec<(usize, u64, Vec<Rule>)> = {
        let rt = RUNTIME.lock();
        rt.agents
            .iter()
            .enumerate()
            .map(|(i, a)| (i, a.memory_id, a.rules.clone()))
            .collect()
    };

    let mut fired: Vec<usize> = Vec::new();

    // Phase 2 — evaluate conditions and apply actions.
    for (idx, memory_id, rules) in &snapshots {
        let actions: Vec<(String, String)> = hive::with_hive(|h| {
            if let Some(mem) = h.memories.get(memory_id) {
                rules
                    .iter()
                    .filter(|r| {
                        mem.blobs
                    ...
                    .map(|r| (r.action_key.clone(), r.action_value.clone()))
                    .collect()
            } else {
                Vec::new()
            }
        ...
        if !actions.is_empty() {
            ...
            hive::with_hive(|h| {
                for (key, val) in actions {
                    h.write_blob(*memory_id, &key, BlobValue::parse(&val));
                }
            });
        }
    }

    // Phase 3 — update trigger counters.
    if !fired.is_empty() {
        let mut rt = RUNTIME.lock();
        for idx in fired {
            if let Some(ag) = rt.agents.get_mut(idx) {
                ag.trigger_count += 1;
            }
        }
    }
